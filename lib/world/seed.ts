...
const initialDrafts: EventDraft[] = [
  ...
  })),
  {
    ...
    actorAgentId: "agt_bruce",
    targetId: "sig_json_server_1709",
    summary: "Bruce reproduced json-server issue #1709 from a clean checkout.",
    tone: "warning",
    payload: {
      signal: {
  ...
  },
  {
    ...
    targetId: "sig_json_server_1709",
    summary: "Wanda reproduced the startup failure on pinned commit 89a34a4.",
    tone: "success",
    payload: {},
  },
  {
    ...
    summary: "Steve proposed validating resource shapes before ID normalization.",
    tone: "active",
    payload: {
      proposal: {
        id: "prp_validate_shape",
        title: "Fail-fast resource validation",
        authorAgentId: "agt_steve",
...
export function createGithubIssueEvents(input: GithubIssueSeed): WorldEvent[] {
  const issueDrafts: EventDraft[] = [
    ...
    })),
    {
      ...
      summary: `Bruce opened discovery for ${input.repository}#${input.issueNumber} after @${input.requestedBy} invoked Code Republic.`,
      tone: "warning",
      payload: {
        signal: {
    ...
    },
    {
      type: "campaign.proposed",
      actorAgentId: "agt_steve",
      targetId: "prp_boundary_normalization",
      summary: "Steve published a boundary-normalization architecture without claiming implementation.",
      tone: "active",
      payload: {
        proposal: {
          id: "prp_boundary_normalization",
          title: "Normalize at the system boundary",
          authorAgentId: "agt_steve",
          summary: "Convert ambiguous external input into the existing domain contract before it reaches shared evaluation logic.",
          tradeoff: "Limits blast radius, but introduces a second normalization layer whose recursion needs explicit design coverage.",
          endorsements: [],
          status: "candidate",
        },
      },
    },
    {
      ...
      summary: "Clint documented equality as shared evaluator semantics without writing implementation code.",
      tone: "info",
      payload: {
        proposal: {
          id: "prp_shared_equality",
          title: "Define equality in the shared evaluator",
          authorAgentId: "agt_clint",
          summary: "Treat primitive leaves as strict equality wherever the shared query evaluator is used.",
          tradeoff: "Creates one consistent rule, but intentionally changes semantics for every internal evaluator caller.",
          endorsements: [],
          status: "candidate",
        },
      },
